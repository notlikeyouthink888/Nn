...
def check_beam(bm, fc, fy, exposure='interior', seismic=True):
    ...
    b, h = sec['b'], sec['h']
    # **العمق الفعّال يُؤخذ من وحدة التصميم نفسها لا يُعاد تقديره هنا.** كان
    # يُحسب بقطر السيخ المختار بينما صُمِّم المقطع بقطر افتراضي، فيخرج d أصغر
    # بمليمترات فيرسب حدّ تباعد الأساور (d/2) بفارق 1 مم — مخالفة وهمية.
    d = bm.get('d') or (h - 40.0 - 10.0 - reb['bottom']['db'] / 2.0)
    out = []
...
# ------------- الأبواب التي أُضيفت بعد قراءة النص المطبوع 318-19 -------------
def check_torsion(t):
    """الالتواء — ACI 318-19 الباب 22.7 مع 9.6.4 و9.7.5 و9.7.6.3."""
    out = []
...
def check_joint(j):
    """قص العقدة جسر–عمود — ACI 318-19 المادة 18.8.4."""
...
def check_deflection(rows):
    """الترخيم — ACI 318-19 جدول 24.2.2 بحالاته الأربع."""
    out = []
    for c in rows:
...
def check_durability(d):
    """الديمومة وأصناف التعرّض — ACI 318-19 الباب 19.3."""
...
def check_ties(cr):
    """تركيب الأتاري والأساور — ACI 318-19 المادتان 25.7.2 و25.3.4."""
...
def check_beam_rebar(b):
    """قطع الحديد ووصله بالجسر — ACI 318-19 المادتان 9.7.3 و18.6.3."""
    c = b['cut']; sp = b['splice']
...
def check_canti(c):
    """الكانتيليفر — ACI 318-19 جدول 9.3.1.1 و9.7.3 و25.4."""
...
def check_btype(bm, slab_h, fy):
    """ما يوجبه **نوع الجسر** تحديداً — بنود لا تنطبق على كل الأنواع.

    الجسر ليس صنفاً واحداً بالكود: الشفة الضاغطة (6.3.2.1) والحدّ الأقصى للعرض
    (18.6.2.1) وأدنى السماكة (جدول 9.3.1.1) وحديد التعليق (R9.7.6.2.2) وحديد
    الجسر العميق الموزّع (9.9.3.1) — كلها بنود يفعّلها أو يُبطلها النوع.
    """
    t = bm.get('btype')
    if not t:
        return []
    sec = bm['section']; reb = bm['rebar']
    b, h = sec['b'], sec['h']
    out = [_row('—', 'نوع الجسر المختار', t['name'], '—', 'ok',
                'موضع البلاطة: %s' % ({'top': 'فوق الجسر — يتدلّى تحت السقف',
                                       'in': 'بمستواه — مخفي داخل السماكة',
                                       'bottom': 'تحته — يصعد فوق السقف'}[t['slab']]))]
    # 18.6.2.1 — الحدّ الأقصى لعرض الجسر بإطار مقاوم للعزوم
    out.append(_cmp('18.6.2.1', 'عرض الجسر مقابل حدّ العقدة', b, t['width_cap'], 'مم',
                    note='c₂ + 2·min(c₂ , 0.75·c₁) — وما زاد لا يصل حمله للعقدة'))
    # 6.3.2.1 — عرض الشفة الفعّال، ولأي إشارة عزم
    fl = t['flange']
    out.append(_row('6.3.2.1', 'عرض الشفة الفعّال bf',
                    '%d مم (%s)' % (int(fl['bf']),
                                    'للعزم الموجب' if fl['pos'] else
                                    'للعزم السالب' if fl['neg'] else 'لا شفة'),
                    'bw + 2·min(8hf , sn/2 , ln/8)', 'ok', fl['note'].replace('**', '')))
    # 9.6.1.2 — الحديد الأدنى يُحسب بعرض **العنق** لا الشفة
    d = h - reb['cover'] - reb['stirrup']['db'] - reb['bottom']['db'] / 2.0
    out.append(_cmp('9.6.1.2', 'الحديد الأدنى محسوباً بعرض العنق bw',
                    reb['bottom']['As'], E.as_min(bm['fc'], fy, b, d), 'مم²', mode='>=',
                    note='المقطع T يُحسب انحناؤه بعرض الشفة، لكن As,min بعرض '
                         'العنق — وإلا خرج الحدّ الأدنى أربعة أضعاف الحقيقي'))
    # 25.7.2.3 — أرجل الأساور من العرض
    out.append(_cmp('25.7.2.3', 'التباعد العرضي بين أرجل الأساور',
                    (b - 80.0) / max(1, t['legs'] - 1), min(max(d, 1.0), 600.0), 'مم',
                    note='%d أرجل — %s' % (t['legs'],
                                           t['legs_rule'].replace('**', ''))))
    # 9.3.1.1 — أدنى سماكة (المخفي يخالفها بطبيعته فيُفحص ترخيمه بالحساب)
    hmin = E.min_h_beam(sec['span'], 'both' if sec['nspan'] > 1 else 'one_end', fy)
    if t['k'] == 'hidden':
        d_ok = min(dd['defl_ok'] for dd in bm['design'])
        out.append(_row('9.3.1.1.1', 'أدنى سماكة — الجسر المخفي',
                        '%d مم' % int(h), '%d مم' % int(hmin),
                        'ok' if d_ok else 'fail',
                        'العمق مقيَّد بسماكة السقف فيخالف الجدول بطبيعته، '
                        'والبند نفسه يسمح بذلك شرط **حساب** الترخيم — وقد '
                        'حُسب و%s' % ('طابق الحدّ' if d_ok else 'تجاوز الحدّ')))
    else:
        out.append(_cmp('9.3.1.1', 'أدنى سماكة بلا حساب ترخيم', h, hmin, 'مم',
                        mode='>=', note='جدول 9.3.1.1'))
    # R9.7.6.2.2 — حديد التعليق بالجسر المقلوب
    hg = reb.get('hanger')
    if hg:
        out.append(_cmp('R9.7.6.2.2', 'Av/s للقصّ + التعليق معاً',
                        hg['Av_s_prov'], hg['Av_s'] + hg.get('Av_s_shear', 0.0),
                        'مم²/م', mode='>=',
                        note='البلاطة تستند على الوجه السفلي فحملها يُعلَّق '
                             'بأساور تعبر كامل العمق — وهذه **تُضاف** على '
                             'أساور القصّ ولا تحلّ محلّها'))
    elif t['slab'] == 'bottom':
        out.append(_row('R9.7.6.2.2', 'حديد التعليق', 'لم يُحسب', '—', 'review',
                        'الجسر مقلوب فيلزمه حديد تعليق — راجع الحمل المعلَّق'))
    # 9.9 — الجسر العميق
    if t['k'] == 'deep':
        w = reb.get('web') or {}
        out.append(_cmp('9.9.1.1', 'نسبة البحر للعمق ln/h (تعريف الجسر العميق)',
                        t['ln'] / (h / 1000.0), 4.0, '',
                        note='ln/h ≤ 4 وإلا فهو جسر عادي لا عميق'))
        # 9.9.3.1 يطلب Av ≥ 0.0025·bw·s **لكامل المقطع** لا لكل وجه، والمنفَّذ
        # موزّع على الوجهين فيُجمع الوجهان ويُقارَن بالمطلوب الواحد.
        out.append(_cmp('9.9.3.1', 'الحديد الموزّع (مجموع الوجهين)',
                        w.get('As_prov', 0.0), w.get('Av_min', 0.0) * 1000.0,
                        'مم²/م', mode='>=',
                        note='Av ≥ 0.0025·bw·s رأسياً و Avh ≥ 0.0025·bw·s₂ أفقياً '
                             '— وتباعدهما ≤ min(d/5 , 300 مم)'))
        out.append(_row('الفصل 23', 'طريقة التصميم', 'دعامة ورباط (Strut-and-Tie)',
                        'إلزامية للجسر العميق', 'review',
                        'انفعال المقطع غير خطّي فتسقط نظرية الانحناء — الحديد '
                        'المعروض حدّ أدنى تفصيلي، والنمذجة تُعمَل بالبرنامج'))
    # 22.7 — الالتواء بالجسر الطرفي
    if t.get('torsion'):
        out.append(_row('22.7', 'الالتواء بالجسر الطرفي', 'إلزامي لهذا النوع',
                        'أساور مغلقة + حديد طولي محيطي', 'review',
                        'البلاطة تستند من جهة واحدة فينقلب عزم طرفها التواءً — '
                        'انظر قسم «الالتواء (الباب 22.7)»'))
    return out
...
def check_piles(a):
    ...
    # **بلا معالجة**. أمّا حين يُنفَّذ الإحلال فالتربة التي أوجبت العمق لم تعد
    # موجودة — حُفرت واستُبدلت، والأساس السطحي يجلس على طبقة مصنّعة.
    rep = a.get('replace') or {}
    mismatch = pl['need'] and a.get('type') != 'piles' and not rep.get('ok')
    out = [_row('13.4', 'هل توجب التربة تأسيساً عميقاً؟',
                ...
                + (' · وقد نُفِّذ **إحلال** بعمق %.1f م فزال سبب العمق: الأساس '
                   'يجلس على طبقة سبيس مدكوك تحمّلها %d kPa لا على تربة الموقع'
                   % (rep.get('depth') or 0, int(rep.get('qa_new') or 0))
                   if rep.get('ok') else ''))]
    if rep and rep.get('depth') is not None:
        out.append(_row('13.1.1', 'الإحلال بدل التأسيس العميق',
                        ('مطبَّق — حفر %.1f م واستبدال بسبيس مدكوك (%d طبقة دكّ)'
                         % (rep['depth'], rep.get('layers', 0))) if rep.get('ok')
                        else 'غير ممكن — ' + (rep.get('why') or [''])[0],
                        'يُفحص قبل اللجوء للركائز',
                        'ok' if rep.get('ok') else 'na',
                        rep.get('spec') if rep.get('ok') else rep.get('note', '')))
    ...
    b = a.get('band') or {}
    if b:
        out.append(_row('13.1.1', 'نطاق تحمّل التربة الحاكم لنوع الأساس',
                        '%s (%s) — %s' % (b.get('soil', ''), b.get('span', ''),
                                          b.get('verdict', '')),
                        'النوع يُشتقّ من النطاق ثم ترفعه المساحة والهبوط ولا تُنزلانه',
                        'ok', b.get('why', '')))
    st = a.get('settle')
    if st:
        deep = a.get('type') == 'piles'
        # حين يكون المنفَّذ ركائز، رسوب الهبوط السطحي ليس مخالفة — هو **سبب**
        # اختيار الركائز، والهبوط على الركائز يُثبت بفحص التحميل لا بهذه المعادلة.
        out.append(_row('13.3.1.1', 'الهبوط الكلي المقدَّر (على أساس سطحي)',
                        '%.1f مم' % st['S'],
                        '≤ %.0f مم (Skempton & MacDonald — حسب التربة ونوع الأساس)' % st['limit'],
                        'na' if deep else ('ok' if st['ok'] else 'fail'),
                        ('آني %.1f + تضاغطي %.1f · %s · سُمك الطبقة الانضغاطية %.1f م'
                         % (st['Si'], st['Sc'], st['src'], st['H']))
                        + (' · والمنفَّذ ركائز، فرسوب هذا الفحص هو سبب اختيارها '
                           'لا مخالفة فيه — وهبوط الركائز يُثبَت بفحص التحميل '
                           '(Pile Load Test)' if deep else '')))
    return out
...
def report(R):
    ...
    for d0, nm in (('x', 'X'), ('y', 'Y')):
        ...
        bt_rows = check_btype(R['beams'][d0], m['slab']['h'], fy)
        if bt_rows:
            secs.append(dict(name='نوع الجسر %s — %s' % (
                nm, R['beams'][d0]['btype']['name']), rows=bt_rows))

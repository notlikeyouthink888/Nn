...
def schedule(R):
    ...
    # --------------------------- الكانتيليفر ---------------------------
    # حديد البروز **كله علوي**، ويخرج بصفّ مستقل بالجدول حتى لا يختلط بحديد
    # السقف: طوله = البروز + طول النشر داخل البحر الخلفي، وينتهي بعكفة 90° هابطة.
    ca = R.get('canti') or {}
    if ca.get('on'):
        for it in ca['items']:
            n_t = D.n_bars(it['edge'], (it['top'].get('s') or 200) / 1000.0)
            n_b = D.n_bars(it['edge'], (it['bottom'].get('s') or 200) / 1000.0)
            ld_ = min(it['anchor']['ld'], it['back_span'] * 0.9)
            hk_ = RB.hook_full(it['top']['db'], 90, 'bar')
            rows.append(_row('CA-T%s' % it['side'].upper().replace('+', 'P').replace('-', 'M'),
                             'كانتيليفر %s — الحديد **العلوي** (الشدّ)' % it['side_name'],
                             it['top']['db'], 'مستقيم بعكفة 90° هابطة عند الطرف الحرّ',
                             it['L'] + ld_, n_t * nf, fc, fy, top=True,
                             extra=hk_['added'] / 1000.0,   # عكفة الطرف الحرّ لكل سيخ
                             note='ACI 9.7.3 — عزم البروز سالب على طوله كله فالشدّ '
                                  'بالأعلى · الطول = بروز %.2f + نشر داخل البحر الخلفي '
                                  '%.2f م · العكفة %d مم' % (it['L'], ld_, int(hk_['added']))))
            rows.append(_row('CA-B%s' % it['side'].upper().replace('+', 'P').replace('-', 'M'),
                             'كانتيليفر %s — سفلي (انكماش وتماسك)' % it['side_name'],
                             it['bottom']['db'], 'مستقيم بعكفة', it['L'] + 0.3,
                             n_b * nf, fc, fy,
                             note='ACI 7.6.1.1 حديد انكماش 0.0018·Ag + 9.7.7 تماسك — '
                                  'الوجه السفلي وجه ضغط ولا يقاوم عزم البروز'))

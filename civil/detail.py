...
# ================================ الكراسي ================================
CHAIRS = [
    ('z90', 'كرسي Z بزاوية 90°', 'الشكل المنفَّذ بالموقع العراقي: سيخ واحد '
     'يُثنى ستّ ثنيات **بمستويين**: قدم باتجاه Z ← رجل عمودية ← عرضة باتجاه X '
     '← رجل ← قدم بالاتجاه المعاكس. فالمسقط حرف Z والكرسي لا ينقلب على جنبه'),
    ('s135', 'كرسي مائل 135°', 'نفس الثني بمستويين لكن الرجلين بميل 45° — '
     'أثبت جانبياً لكنه يأخذ عرضاً يساوي ارتفاعه بكل جهة'),
...
#: القدم = الجزء الذي يستند على الشبكة السفلى ويُربط بها بالسلك، وهو **عمودي
#: على العرضة** (باتجاه Z حين تمتدّ العرضة باتجاه X). وطولها الأدنى 150 مم
#: ولا يقلّ عن تباعد الشبكة السفلى حتى تعبر سيخاً واحداً منها على الأقل،
#: ويُضاف عليه **الطرف الطافر** 60 مم الذي يبرز خارج ذلك السيخ.
FOOT_MM = 150.0
...
def chair(kind, height, db=None, top_run=250.0, foot=FOOT_MM, s_top=None, s_bot=None):
    ...
    الشكل هو الكرسي المصنوع بالموقع من سيخ واحد، وهو **مثنيّ بمستويين لا
    بمستوى واحد**:

        قدم A باتجاه Z ← ثنية 90° بمستوى Y–Z ← رجل صاعدة ← ثنية 90° بمستوى
        X–Y ← عرضة باتجاه X ← رجل نازلة ← قدم B باتجاه Z المعاكس

    فتكون القدمان **عموديتين على العرضة**، والمسقط حرف Z. والكرسي المسطّح
    بمستوى واحد ينقلب على جنبه بأول وطأة قدم أو خرطوم صبّ، ولا يقاوم شيئاً
    بالاتجاه العمودي على مستواه. والقدمان المتعاكستان تعملان مزدوجاً يقاوم
    الدوران حول محور العرضة.

    ولكل قدم **طرف طافر** (`tail`) يبرز خارج آخر سيخ تعبره: هو ما يُمسَك به
    ويُربَط، وبدونه يقف طرف السيخ على السيخ نفسه فينزلق.

    والمقادير الثلاثة المحسوبة لا المقدَّرة:

      * `rise` = ارتفاع **محور** العرضة فوق **محور** القدم = الصافي ناقص قطر
        السيخ. لأن ظهر القدم يلامس الشبكة السفلى فمحورها يعلوها بنصف قطر،
        وظهر العرضة يلامس الشبكة العلوية فمحورها دونها بنصف قطر.
      * `top_run` ≥ تباعد الشبكة **العلوية**: العرضة تعبر سيخاً علوياً أينما
        وقعت (الأسياخ العلوية عمودية عليها).
      * `foot` ≥ تباعد الشبكة **السفلى** + الطرف الطافر: كل قدم تعبر سيخاً
        سفلياً واحداً على الأقل فتُربط به.

    فيصير الكرسي مربوطاً بـ **ثلاثة مواضع**: العرضة وقدمان.
    """
    height = max(50.0, height)
    db = db or (10.0 if height <= 300 else 12.0)
    tail = 60.0                                      # الطرف الطافر خارج آخر سيخ
    # العرضة تعبر سيخاً علوياً مهما وقعت · وكل قدم تعبر سيخاً سفلياً
    top_run = max(float(top_run or 250.0), float(s_top or 0.0))
    foot = max(float(foot or FOOT_MM), 150.0, float(s_bot or 0.0)) + tail
    ...
    if kind == 's135':
        ...
        leg = rise
        n_bend, ang, foot = 6, 90, max(foot, 180.0)
        run = 0.0
    else:                                            # z90 — شكل الموقع
        leg = rise
        n_bend, ang = 6, 90
    ...
    half_w = (top_run / 2.0 + run + db / 2.0) / 1000.0   # نصف الامتداد بمحاذاة العرضة (X)
    half_d = (foot + db / 2.0) / 1000.0                  # نصف الامتداد عمودياً عليها (Z)
    return dict(kind=kind, name=CHAIR_MAP.get(kind, CHAIRS[0])[1], height=height, db=db,
                ...
                tail=tail, run=run, len_each=length, half_w=half_w, half_d=half_d,
                width=2 * half_w, depth=2 * half_d, s_top=s_top, s_bot=s_bot,
                shape='مثنيّ بمستويين: القدمان باتجاه Z والعرضة باتجاه X — '
                      'المسقط حرف Z، فلا ينقلب على جنبه كما ينقلب المسطّح',
                anchor='ظهر القدمين على الشبكة السفلى · ظهر العرضة تحت الشبكة '
                       ...
                       % (int(db / 2), int(db / 2)),
                ties=3,
                label='%s Ø%d ارتفاع صافٍ %d مم (%d ثنيات · %d°)' % (
                    CHAIR_MAP.get(kind, CHAIRS[0])[1], int(db), int(height), n_bend, ang))
...
def chair_layout(Lx, Ly, h, cov_top, cov_bot, db_top, db_bot, kind='z90',
    ...
    # الهامش يختلف بالاتجاهين: امتداد العرضة بمحور X وامتداد القدمين بمحور Z.
    mg = cov_bot / 1000.0 + c['half_w']            # بمحاذاة العرضة
    mgt = cov_bot / 1000.0 + c['half_d']           # بمحاذاة القدمين

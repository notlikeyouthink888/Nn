...
#: تباعد الكراسي بالاتجاهين — متر. شبكة منتظمة خطوتها **متر واحد بالضبط**،
#: وهي القاعدة المنفَّذة بالعراق والحدّ الأدنى من مدى CRSI (1.0–1.5 م): ما زاد
#: عليها يجعل الشبكة العلوية تترخّم بين الكراسي فينقص الغطاء أو العمق الفعّال.
CHAIR_SP = 1.0


def chair_rows(length, spacing=CHAIR_SP, margin=0.0):
    """عدد الكراسي على طول معيّن بشبكة خطوتها **ثابتة** مركزها منتصف الطول.

    `floor` لا `ceil`: الخطوة مقدّسة (متر بالضبط) والطول هو الذي يُقصّ عليها،
    فالكرسي الذي لا يتسع لا يُضاف بخطوة أصغر.

    و`margin` هو ما يُقتطع من كل طرف: الغطاء الخرساني + **نصف عرض الكرسي**،
    لأن الكرسي جسم له عرض ولا يجوز أن يبرز طرف قدمه خارج الخرسانة. والعدّ
    من مركز الطول لا من طرفه لأن الشبكة بالمجسّم مركزها مركز المبنى — وبهذا
    يطابق العدد المكتوب العدد المرسوم بالضبط."""
    a = max(0.0, (float(length) - 2.0 * float(margin)) / 2.0)
    return 2 * int(math.floor(round(a / spacing, 6))) + 1


def chair(kind, height, db=None, top_run=250.0, foot=FOOT_MM, s_top=None, s_bot=None):
    """هندسة الكرسي وطول قطعته — `height` هو **الارتفاع الصافي** بالمليمتر:
    من **ظهر الشبكة السفلى** إلى **بطن الشبكة العلوية**، لا من محور لمحور.

    المرجع الشكلي هو الكرسي المنفَّذ بالموقع: سيخ واحد بأربع ثنيات، قدمان
    أفقيتان مثنيّتان للخارج تستندان على الشبكة السفلى، ورجلان عموديتان،
    وعرضة علوية تحمل الشبكة العلوية.

    ثلاثة مقادير تُحسب هنا ولا تُقدَّر:

      * `rise` = ارتفاع **محور** العرضة فوق **محور** القدم = الصافي ناقص قطر
        السيخ. لأن ظهر القدم يلامس الشبكة السفلى فمحورها يعلوها بنصف قطر،
        وظهر العرضة يلامس الشبكة العلوية فمحورها دونها بنصف قطر. وبلا هذا
        الطرح يتداخل الكرسي بالشبكتين بقطر كامل.
      * `top_run` ≥ تباعد الشبكة العلوية: العرضة يجب أن تعبر **سيخاً علوياً
        واحداً على الأقل** أينما وقعت، وإلا سقطت بين سيخين ولم تحمل شيئاً.
        وهذا ما يغني عن «تثبيت» الكرسي على خط سيخ ويُبقي الشبكة منتظمة متراً.
      * `width` ≥ تباعد الشبكة السفلى: القدمان كذلك يجب أن تعبرا سيخاً سفلياً.
    """
    height = max(50.0, height)
    db = db or (10.0 if height <= 300 else 12.0)
    # العرضة تعبر سيخاً علوياً مهما وقعت
    top_run = max(float(top_run or 250.0), float(s_top or 0.0))
    rise = max(20.0, height - db)                    # محور القدم ← محور العرضة
    if kind == 's135':
        leg = rise * math.sqrt(2.0)                  # ميل 45°
        n_bend, ang = 4, 135
        run = rise                                    # الإزاحة الأفقية للرجل المائلة
    elif kind == 'sb':
        leg = rise * 1.15
        n_bend, ang = 6, 90
        top_run = max(top_run, 300.0)
        run = 0.0
    elif kind == 'ihc':
        leg = rise
        n_bend, ang, foot = 4, 90, max(foot, 120.0)
        run = 0.0
    else:                                            # z90 — شكل الموقع
        leg = rise
        n_bend, ang = 4, 90
        run = 0.0
    # القدمان تعبران سيخاً سفلياً: نصف العرض لا يقلّ عن نصف تباعد الشبكة السفلى
    if s_bot:
        foot = max(foot, float(s_bot) / 2.0 - top_run / 2.0 - run)
    ...
    return dict(kind=kind, name=CHAIR_MAP.get(kind, CHAIRS[0])[1], height=height, db=db,
                leg=leg, rise=rise, angle=ang, bends=n_bend, top_run=top_run, foot=foot,
                run=run, len_each=length, half_w=half_w,
                width=2 * half_w, s_top=s_top, s_bot=s_bot,
                anchor='ظهر القدمين على الشبكة السفلى · ظهر العرضة تحت الشبكة '
                       'العلوية — بلا فراغ وبلا تداخل (محور القدم يعلو الشبكة '
                       'السفلى %d مم ومحور العرضة دون العلوية %d مم = نصف القطر)'
                       % (int(db / 2), int(db / 2)),
                label='%s Ø%d ارتفاع صافٍ %d مم (زاوية %d°)' % (
                    CHAIR_MAP.get(kind, CHAIRS[0])[1], int(db), int(height), ang))

def chair_layout(Lx, Ly, h, cov_top, cov_bot, db_top, db_bot, kind='z90',
                 spacing=CHAIR_SP, s_top=None, s_bot=None):
    """توزيع الكراسي على مساحة: العدد والارتفاع والوزن.

    الارتفاع الصافي = السماكة − الغطاءين − قطر الشبكة العلوية − قطر طبقتَي
    الشبكة السفلى. وهو بالضبط الخلوص بين ظهر السفلى وبطن العلوية، أي المسافة
    التي يحفظها الكرسي — ومنها يخرج **العمق الفعّال d** للسقف كله.
    """
    ht = max(60.0, h - cov_top - cov_bot - db_top - db_bot)
    c = chair(kind, ht, s_top=s_top, s_bot=s_bot)
    # الهامش يختلف بالاتجاهين: الكرسي عريض بمستوى عرضته ونحيل عمودياً عليه.
    mg = cov_bot / 1000.0 + c['half_w']            # بمحاذاة العرضة
    mgt = cov_bot / 1000.0 + c['db'] / 2000.0      # عمودياً عليها
    nx = chair_rows(Lx, spacing, mg)
    ny = chair_rows(Ly, spacing, mgt)
    n = nx * ny
    ...
    c.update(n=n, nx=nx, ny=ny, spacing=spacing, weight=wt,
             ...
             note='شبكة منتظمة خطوتها **%.2f م بالضبط** بالاتجاهين X وY — '
                  '%d × %d = %d كرسياً. الكرسي يحفظ خلوصاً صافياً %d مم بين '
                  'الشبكتين، وهو ما يحدّد العمق الفعّال d. (CRSI 1.0–1.5 م)'
                  % (spacing, nx, ny, n, int(ht)))
    return c

...
const expSelects = () => EXP_CATS.map(([p, nm], i) => {
  const rows = (META.exposure_classes || []).filter(c => c.code[0] === p);
  const def = (META.exposure_default || [])[i] || (rows[0] || {}).code;
  return S(nm + ' (' + p + ')', 'w_exp' + p,
    rows.map(c => [c.code, c.code + ' — ' + c.desc +
...
PAGES.ref = {
  ...
  html: () => `<div class="grid g2">
    ...
    <div class="card ref"><h3>تصميم الخرسانة — ACI 318-19</h3><ul>
      ...
      <li>طول الرباط ld وفق <b>25.4.2.3(a)</b> ومعاملات <b>جدول 25.4.2.4</b>:
        ψt=1.3 علوي · ψe=1.5/1.2 مطلي · ψs=0.8 <b>لقضيب رقم 19 فأصغر (db ≤ 19.1 مم)</b>
        فـ Ø20 يأخذ 1.0 · وحاصل ψt·ψe ≤ 1.7.</li>
      <li>العكفة ldh وفق <b>25.4.3.1</b> ومعاملات <b>جدول 25.4.3.2</b>: ψc=0.7 بغطاء ≥65مم ·
        ψr=0.8 لعكفة 90° مُطوَّقة — وتُحسب معها صيغة 2019 ويُؤخذ الأكبر.</li>
      ...
      <li><b>الالتواء (22.7)</b>: عتبة <code>Tth = 0.083λ√f'c·Acp²/pcp</code>، وتصميم بمنظومة
        الجَمَلون الفراغي <code>At/s = Tu/(φ·2Ao·fyt·cotθ)</code> مع <code>Ao = 0.85Aoh</code>
        وθ = 45°، وحديد طولي <code>Aℓ = (At/s)·ph·(fyt/fy)</code>، وحدّ المقطع 22.7.7.1،
        وكانات مغلقة بتباعد ≤ min(ph/8 , 300مم) (9.7.6.3.3).</li>
      <li><b>قص العقدة (18.8.4)</b>: <code>Vn = γλ√f'c·Aj</code> مع γ = 1.7/1.2/1.0 حسب
        الحصر، وφ = 0.85، وقوة الحديد بإجهاد <b>1.25fy</b> (18.8.2.1)، وعكفة العقدة
        <code>ldh = fy·db/(5.4λ√f'c)</code> (18.8.5.1).</li>
      <li><b>الترخيم (جدول 24.2.2)</b>: Ie بطريقة Branson (24.2.3.5)، ومعامل الزمن
        <code>λΔ = ξ/(1+50ρ′)</code> (24.2.4.1.1) مضروباً بالترخيم الفوري <b>للحمل الدائم
        وحده</b>، والحدود L/180 و L/360 و <b>L/480</b> و L/240 حسب الحالة.</li>
      <li><b>الديمومة (19.3)</b>: أصناف التعرّض F·S·W·C (جدول 19.3.1.1) وما تفرضه من
        أدنى f'c وأقصى w/cm ونوع أسمنت (جدول 19.3.2.1) — والأشدّ يحكم حين تجتمع.</li>
    ...
    <div class="card ref"><h3>الفرضيات والحدود</h3><ul>
      ...
      <li>التحليل مستوي ثنائي الأبعاد (2D Frame) — لا يشمل التحليل الديناميكي الطيفي. الالتواء يُصمَّم بالباب 22.7 على الجسر الطرفي من عزم طرف البلاطة (جدول 6.5.2)، لا من تحليل فراغي للالتواء.</li>
...
/* ----- شريط حالة فوق المجسم: على أي شيء بُني هذا المبنى؟ ----- */
function planStrip() {
  const g = (WZ && WZ.grid) || null;
  if (!g) return '';
  const src = g.source || 'auto';
  if (src === 'frame' || src === 'plan') {
    const sp = (g.spans_x || []).slice(0, 5).map(v => nf(v, 2)).join(' / ');
    const sy = (g.spans_y || []).slice(0, 5).map(v => nf(v, 2)).join(' / ');
    return `<div class="pstrip on" id="pstrip">
      <span>📐 <b>مبني على مخططك</b>${PLD && PLD.name ? ': ' + PLD.name : ''} —
        <b>${int(g.cols)}</b> عمود · ${nf(WZ.footprint, 1)} م²${
        sp ? ' · بحور X ' + sp + ' م' : ''}${sy ? ' · Y ' + sy + ' م' : ''}${
...
function v3bar(o) {
  o = o || {};
  const fl = o.floors || 1;
  return planStrip() + `<div class="v3bar">
...
function extraPanel(r) {
  ...
  return `
    <div class="card"><h3>🌀 الالتواء بالجسر الطرفي — ACI 318-19 الباب 22.7</h3>
      ${t.required ? table(['البند', 'القيمة'], [
        ...
        ['عزم الالتواء Tu', '<b>' + nf(t.Tu, 2) + ' kN·م</b> — من عزم طرف البلاطة ' +
          nf(t.t_per_m, 2) + ' kN·م/م (معامل ACI 6.5.2 = wu·ℓn²/24)'],
        ['عتبة الإهمال φTth (22.7.1.1)', nf(t.phiTth, 2) + ' kN·م — Tu أكبر منها فالالتواء يُصمَّم'],
        ['عزم التشقق φTcr (22.7.5.1)', nf(t.phiTcr, 2) + ' kN·م — ' + t.case],
        ['حدّ المقطع (22.7.7.1أ)', nf(t.lhs, 2) + ' ≤ ' + nf(t.rhs, 2) + ' ميغا ' + y(t.ok)],
        ['الأساور بعد الالتواء', '<b>' + t.stirrup_new.label + '</b> — التباعد ≤ ph/8 = ' +
          int(t.s_max) + ' مم (9.7.6.3.3)'],
        ['الحديد الطولي للالتواء', '<b>' + t.long_bars.label + '</b> — Aℓ = ' + int(t.Al) +
          ' مم² (الأدنى ' + int(t.Al_min) + ' مم² بالمادة 9.6.4.3)'],
      ]) : `<div class="note">Tu = ${nf(t.Tu, 2)} kN·م أقل من عتبة φTth = ${nf(t.phiTth, 2)} kN·م،
        فالمادة 22.7.1.1 تجيز إهمال الالتواء نصّاً.</div>`}
      <div class="note">البلاطة تستند على الجسر الطرفي من جهة واحدة فقط، فعزم طرفها ينقلب
        <b>التواءً</b> على الجسر — وهذا ما يشقّق الجسور الطرفية والشناشيل. الأساور هنا
    ...
    <div class="card"><h3>🔗 قص العقدة جسر–عمود — ACI 318-19 المادة 18.8.4</h3>
      ...
      <div class="note">قوة الحديد العلوي عند وجه العقدة تُحسب بإجهاد <b>1.25·fy</b> لا fy
        (المادة 18.8.2.1) لأن الحديد يتجاوز حدّ الخضوع بالزلزال: T = ${int(j.T)} kN، وقص العمود
        المقابل ${int(j.Vcol)} kN. وطول نشر العكفة داخل العقدة
        <b>ldh = ${int(j.hook.ldh)} مم</b> = fy·db/(5.4λ√f'c) بالمادة 18.8.5.1 — أطول من
    ...
    <div class="card"><h3>📉 الترخيم — ACI 318-19 جدول 24.2.2 بحالاته الأربع</h3>
      ${dd.rows.map(c => `<h4 style="margin:8px 0 4px;color:var(--mut);font-size:13px">جسر ${c.beam}
        ...
        ${table(['الحالة', 'ما يُفحص', 'المحسوب (مم)', 'الحدّ (مم)', 'الحالة'],
          c.cases.map(o => [o.label, o.what, nf(Math.abs(o.value), 1),
            'L/' + int(o.den) + ' = ' + nf(o.limit, 1), y(o.ok)]))}`).join('')}
      <div class="note">كان الفحص سابقاً بحدّ واحد (L/240) وبمعامل زمن ثابت 2.0 على الترخيم
        <b>الكلي</b>. الآن: λΔ = ξ/(1+50ρ′) يُضرب بالترخيم الفوري للحمل <b>الدائم وحده</b>
        (المادة 24.2.4.1.1)، ويُقارَن بالحالة الحاكمة. الافتراضي «يحمل عناصر تتضرّر» بحدّ
    ...
    <div class="card"><h3>🧪 الديمومة وأصناف التعرّض — ACI 318-19 الباب 19.3</h3>
      ...
      <div class="grid g4" style="margin-top:10px">
        ${kpi("f'c المُدخَل", int(du.fc) + ' ميغا', du.ok_fc ? 'ok' : 'bad')}
        ${kpi("أدنى f'c للديمومة", int(du.fc_min) + ' ميغا', du.ok_fc ? 'ok' : 'bad')}
        ${kpi('أقصى w/cm', du.wcm_max ? nf(du.wcm_max, 2) : '—')}
        ${kpi('الصنف الحاكم', du.gov_fc)}</div>
      <div class="note">${du.ok_fc
        ? "المقاومة المُدخَلة تفي بمتطلبات الديمومة لأصناف التعرّض المختارة."
        : `<b style="color:#f87171">المقاومة المُدخَلة أقل من الحدّ الكودي بـ ${int(du.fc_gap)} ميغا.</b>
           ارفع f'c إلى ${int(du.fc_min)} ميغا، أو غيّر أصناف التعرّض من بطاقة «التربة والمناسيب»
           إن كان فحص تربتك يثبت كبريتات أقل.`}
        <br>هذا الباب لا علاقة له بالحمل: الكود يفرض حداً أدنى للمقاومة وحداً أعلى لنسبة
        الماء/الأسمنت <b>بسبب البيئة</b>. وحين تجتمع الأصناف تُؤخذ <b>الأشدّ</b> (المادة 19.3.2.1).
        تربة العراق الجبسية غالباً S2، وهي التي تأكل الأساس من تحت لا الحمل.</div></div>`;
}
...
/* ================= استيراد مخطط DWG/DXF وتطبيقه ================= */
let PLR = null;          // العناصر الخام من المتصفح
let PLD = null;          // نتيجة التحليل من السيرفر
const ROLE_AR = { col: 'أعمدة', wall: 'جدران', axis: 'محاور', other: 'عرض فقط', off: 'تجاهل' };

const planWhy = () => `<ul style="padding-right:18px;font-size:12.5px;color:var(--mut);line-height:1.9">
  <li>القراءة تتم <b>داخل متصفحك</b> — الملف لا يُرفع لأي خدمة خارجية. محرّك DWG
      يُحمَّل مرة واحدة (~2.3 ميغا مضغوط) ثم يُخزَّن بالمتصفح، وإن تعذّر صدّر المخطط DXF.</li>
  <li>يُستخرج منه <b>مواقع الأعمدة</b> و<b>الجسور</b> و<b>المحاور</b> و<b>حدّ البناء</b>
      و<b>الدرج والمصاعد</b>، والمقياس من <b>الأبعاد المكتوبة</b> بالمخطط نفسه.</li>
  <li>الملف الواحد قد يحوي <b>عدة مخططات</b> جنب بعض — تُفصل كلها ويُختار صاحب أكثر أعمدة،
      وتقدر تبدّله من قائمة «المخطط داخل الملف».</li>
  <li>بمجرد الرفع <b>يُعاد تصميم المشروع</b> على أعمدتك الحقيقية — لا حاجة لزر،
      ويبقى المخطط ظاهراً خلفيةً تحت المجسم للمقارنة.</li>
</ul>`;
...
async function loadPlanFile(inp) {
  ...
  try {
    ...
    PLR = await PlanIO.read(f, msg);
    msg('تحليل ' + PLR.ents.length + ' عنصر…');
    PLD = await post('plan/parse', PLR);
    PLD.name = f.name;
    renderPlan();
    drawPlan();
    ...
    if (fr && fr.n_cols >= 4) {
      ...
      msg('⚠️ قرأت الملف لكن ما تعرّفت على أعمدة كافية (' + ((fr && fr.n_cols) || 0) + ') — '
        + 'صحّح المقياس أو المخطط المختار من الشريط فوق، أو دور طبقة الأعمدة من «🗂️ طبقات المخطط» '
        + 'أدناه، ثم اضغط «✅ طبّق على المشروع». '
        + 'المشروع الحالي بقي كما هو.');
    }
  } catch (e) {
    console.error(e);
    msg('✗ ' + (e.message || e));
...
/* يعيد التحليل بعد تغيير الأدوار أو المقياس أو المخطط المختار */
...
function drawPlan() {
  ...
  if (cols.length) {
    ...
    oy = (Math.min(...cols.map(c => c.y)) + Math.max(...cols.map(c => c.y))) / 2 / PLD.scale;
  } else if (PLD.extents) {
  ...
  V3.planXform({ level: +($('#pl_lvl') ? $('#pl_lvl').value : 0),
                 ...
                 op: +($('#pl_op') ? $('#pl_op').value : 0.85) });
  if (!PLAN_ON) { PLAN_ON = true; tgl('btnPlan', true); }
  V3.plan(true);
  if (!XRAY_ON) toggleXray();              // الخلفية تُرى عبر السقوف
  const e = $('#pl_stat');
  if (e) e.textContent = st.layers + ' طبقة مرسومة · ' + int(st.segs) + ' قطعة خطية';
}

function renderPlan() {
  const host = $('#wp_plan');
  if (!host) return;
  if (!PLD) { host.innerHTML = planHead(); return; }
  const g = PLD.grid, b = PLD.boundary, s = PLD.suggested;
  const nfl = (WZ && WZ.model.floors) || 1;
  const applied = !!(window.__frame_override || window.__grid_override);
  const stH = (WZ && WZ.model && WZ.model.story_h) || 3.2;
  host.innerHTML = `
    <!-- ①  شريط القرار: كل ما يُغيّر المشروع بأول الصفحة -->
    <div class="pdec">
      <div class="pdec-r">
        <span class="pdec-f">📄 <b>${PLD.name}</b></span>
        <span class="tag ${applied ? 't-ok' : 't-warn'}">${applied ? 'مطبَّق على المشروع ✓' : 'غير مطبَّق'}</span>
        <span class="sp"></span>
        <button class="btn" onclick="applyPlan()">✅ طبّق على المشروع</button>
        ${applied ? `<button class="btn gh" onclick="clearPlanGrid()">↩️ رجّع التلقائي</button>` : ''}
        <button class="btn gh" onclick="togglePlan()">👁️ الخلفية</button>
        <button class="btn gh" onclick="V3&&V3.reset();$('#v3d').scrollIntoView({behavior:'smooth',block:'center'})">🧊 للمجسم</button>
      </div>
      <div class="pdec-r">
        <label>المخطط داخل الملف</label>
        <select id="pl_ix" onchange="replan({plan_index:+this.value})">
          ${(PLD.plans || []).map(p => `<option value="${p.i}" ${p.i === PLD.plan_index ? 'selected' : ''}>
            #${p.i + 1} ${p.name ? '— ' + p.name : ''} · ${p.n} عمود · ${nf(p.w, 1)}×${nf(p.h, 1)} م</option>`).join('')}
        </select>
        <label>المقياس</label>
        <select id="pl_sc" onchange="replan({scale:+this.value})">
          ${(PLD.scales || []).map(x => `<option value="${x.v}" ${Math.abs(x.v - PLD.scale) < 1e-9 ? 'selected' : ''}>
        ...
        <input type="file" id="pl_file" accept=".dwg,.dxf" onchange="loadPlanFile(this)"
          style="width:auto;font-size:11.5px">
      </div>
      <div id="pl_msg" class="pdec-m">✓ ${int(PLD.n_ents)} عنصر · ${PLD.layers.length} طبقة ·
        ${(PLD.plans || []).length} مخطط بالملف</div>
      ${(PLD.warnings || []).length ? `<div class="pdec-m" style="color:#fde68a">
        ${PLD.warnings.map(w => '⚠️ ' + w).join(' · ')}</div>` : ''}
    </div>
    ...
    ${g ? `<div class="rec" style="margin-top:12px">
      <h3>🧭 ما استُخرج من مخططك وبُني عليه</h3>
      <ul>
        ...
        <li>أقرب شبكة منتظمة للمقارنة: ${g.nx} × ${g.ny} بحر بمسافة ${nf(g.sx, 2)} × ${nf(g.sy, 2)} م ·
            ...
            ${g.regular ? 'أعمدتك منتظمة فعلاً ✓'
              : '<b>التصميم لا يُبنى على هذه الشبكة</b> بل على مواقع أعمدتك الحقيقية.'}</li>
      ...
      </ul></div>` : `<div class="note" style="margin-top:12px">
      ما قدرت أستخرج شبكة — حدّد طبقة الأعمدة الصحيحة من «طبقات المخطط» أدناه، أو صحّح المقياس فوق.</div>`}

    <!-- ③  التفاصيل الطويلة مطوية -->
    <details class="fold" style="margin-top:12px"><summary>🎛️ ضبط عرض الخلفية (دوران · شفافية · منسوب)</summary>
      <div class="v3bar" style="margin-top:8px">
        <b style="font-size:12px;color:var(--mut)">دوران:</b>
        <input type="range" id="pl_rot" min="-180" max="180" step="1" value="0" style="width:120px"
          oninput="V3&&V3.planXform({rot:+this.value});$('#pl_rotv').textContent=this.value+'°'">
        <span id="pl_rotv" style="font-size:11px;color:var(--acc2)">0°</span>
        <b style="font-size:12px;color:var(--mut)">شفافية:</b>
        <input type="range" id="pl_op" min="0.15" max="1" step="0.05" value="0.85" style="width:100px"
          oninput="V3&&V3.planXform({op:+this.value})">
        <b style="font-size:12px;color:var(--mut)">المنسوب:</b>
        <select id="pl_lvl" onchange="V3&&V3.planXform({level:+this.value})" style="width:auto;padding:6px 10px">
          <option value="0">الأرضي</option>
          ${Array.from({ length: nfl }, (_, i) => `<option value="${(i + 1) * stH}">سقف طابق ${i + 1}</option>`).join('')}
        </select>
        <button class="btn gh" style="padding:6px 12px" onclick="drawPlan()">↻ أعد الرسم</button>
    ...
    <details class="fold"><summary>🟦 الأعمدة المكتشَفة (${(PLD.columns || []).length})</summary>
...
async function clearPlanGrid() {
  ...
  if (window.__area_before != null) {
    $('#w_area').value = window.__area_before; $('#w_cov').value = window.__cov_before;
    window.__area_before = window.__cov_before = null;
  }
  await PAGES.wizard.run();
  setTimeout(() => { if (PLD) { wtab('plan'); drawPlan(); } showChange(); }, 500);
}

/* بعد أي إعادة بناء: انقل المستخدم للمجسم ووَمِّض شريط الحالة ليرى التغيير بعينه */
function showChange() {
  const v = $('#v3d'), s = $('#pstrip');
  if (v) v.scrollIntoView({ behavior: 'smooth', block: 'center' });
  if (s) { s.classList.add('flash'); setTimeout(() => s.classList.remove('flash'), 2200); }
}

async function setRole(name, role) {
  const l = (PLD.layers || []).find(x => x.name === name);
  if (l) l.role = role;
  await replan();
}
...
async function applyPlan() {
  ...
  // نحفظ مدخلات المستخدم الأصلية ليرجعها زر «رجّع التلقائي» كما كانت بالضبط
  if (window.__area_before == null) {
    window.__area_before = $('#w_area').value; window.__cov_before = $('#w_cov').value;
  }
  ...
  await PAGES.wizard.run();
  setTimeout(() => { wtab('plan'); drawPlan(); showChange(); }, 500);
}
...
PAGES.wizard = {
  ...
  <div class="grid g2">
    <div class="card"><h3>١ · القطعة والبناء</h3><div class="f">
      ...
      ${F("f'c", 'w_fc', 31, 1, 'MPa')}${F('fy', 'w_fy', 420, 10, 'MPa')}
      ${S('المحافظة', 'w_city', META.cities.map(c => c.name), 'بغداد')}</div>
      <div class="hint">الافتراضي f'c = <b>31 ميغا</b> لا 25، لأن أصناف التعرّض الافتراضية
        (كبريتات S2 — تربة العراق الجبسية) توجبه بجدول ACI 19.3.2.1. أنزله إلى 25 فقط إن
        كان فحص تربتك يثبت كبريتات أقل، وغيّر صنف S معه من بطاقة «التربة والمناسيب».</div></div>
    <div class="card"><h3>٢ · التربة والمناسيب</h3><div class="f">
      ...
      <div class="hint">الكود يفرض حدّاً أدنى لـ f'c وحدّاً أعلى لنسبة الماء/الأسمنت
        <b>بسبب البيئة لا الحمل</b> (جدول 19.3.2.1). تربة العراق الجبسية غالباً
        صنف كبريتات S2 فتوجب f'c ≥ 31 ميغا و w/cm ≤ 0.45 — هذا هو الباب الذي
        يقرّر عمر المبنى، لا مقاومته.</div></div>
  ...
  run: async () => {
    ...
    Object.keys(WT).forEach(k => WT[k] = 0);   // التبويبات تُعاد بناؤها مع النتيجة
...
/* --------- التحليل الإنشائي المتطور (وحدة مستقلّة في fem3d.js) ---------- */
PAGES.fem = {
  ic: '🧮', name: 'التحليل المتطور', ttl: 'التحليل الإنشائي المتطور',
  sub: 'إطار فراغي بطريقة الصلابة المباشرة وفق ACI 318-19 — مصفوفة 12×12 · مخططات M₃₃ و M₂₂ و T و V بالفضاء · إثبات كامل للحساب',
  desc: 'تحليل مصفوفي فراغي بمخططات ثلاثية الأبعاد وإثبات رياضي',
  html: () => (window.FEM3D ? FEM3D.page.html()
    : `<div class="card">وحدة التحليل المتطور غير محمَّلة — تأكّد من تحميل <code>/fem3d.js</code>.</div>`),
...
PAGES.femproj = {
  ...
  sub: 'الإطار الفراغي لمشروعك كما بناه المعالج — ببحوره ومقاطعه وأحماله الحقيقية · عزوم كاملة وتراكيب ACI 318-19 وأساسات',

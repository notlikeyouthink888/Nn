...
function Viewer3D(el, M, onPick) {
  ...
  function buildRebar() {
    ...
    const drawBeam = (cX, cZ, len, bw, hB, z, reb, dir, floor, ttl, det, ends, opt) => {
      ...
      /* السوار **يلتفّ حول مقطع الجسر**، أي بالمستوى العمودي على محوره.
         و`addRings` يبني الحلقة أفقياً (العرض على X والعمق على Z) ثم يديرها:
           rot='x' ⇒ دوران واحد  ⇒ العرض يبقى على X والعمق يصير على Y
           rot='z' ⇒ دورانان     ⇒ العرض يصير على Z والعمق على Y
         فالجسر الماشي على X يحتاج **عرضه على Z** أي rot='z'، والماشي على Z
         يحتاج عرضه على X أي rot='x'. وكان معكوساً: الأساور تخرج بمستوى
         يحوي محور الجسر نفسه — حلقات مستلقية **بطول** الجسر لا ملتفّة حوله.
         (ودليله بالقياس: مقطع السوار كان يمتدّ على X وY لجسرٍ ماشٍ على X.)
         والعرض والعمق يُمرَّران الآن بترتيب ثابت لا يتبدّل مع الاتجاه. */
      addRings(bw - 2 * cv, hB - 2 * cv,
        ...
        null, null, dir === 'x' ? 'z' : 'x', Math.max(6 * sdb.db / 1000, .075),
        // الأتاري الداخلية أرجل **شاقولية** تعبر العمق وتتوزّع على العرض،
        // فهي على محور العرض لا العمق. وكانت مستلقية بعرض المقطع.
        { x: Math.max(0, bLegs), z: 0 });
  ...
  function buildSite() {
    ...
    /* ٢) طبقات السبيس المدكوكة تحت الأساس — **قبل الضنبان لا بعده**

       التسلسل بالموقع: حفر ← دكّ القاع ← فرش السبيس **طبقةً طبقة** ودكّ كل
       طبقة ← ضنبان ← حديد ← صبّ. وكان المشهد يقفز من الحفر إلى الضنبان
       مباشرةً، فيبدو الأساس جالساً على التربة الطبيعية كما هي.
    ...
    const subAreas = (digKind === 'raft' && rf)
      ? [{ x: 0, z: 0, w: rf.Lx + 2 * workW, d: rf.Ly + 2 * workW }]
      : digKind === 'piles'
        ? [{ x: 0, z: 0, w: L + 2 * workW, d: B + 2 * workW }]
        : fts.map(f => ({ x: f.x, z: f.z, w: f.B + .6, d: f.B + .6 }));
    let subV = 0;
    subAreas.forEach(ar => { subV += ar.w * ar.d * subT; });
    subAreas.forEach((ar, ai) => {
      for (let li = 0; li < nLift; li++) {
        const yc = fb - bl - subT + li * liftT + liftT / 2;
        C.slab(gSub, ar.w, liftT * .94, ar.d, ar.x, yc, ar.z,
          C.mat('sand', { rx: ar.w / 1.2, clip: clip }),
          (ai || li) ? null : { title: 'طبقات السبيس المدكوكة', kind: 'site', grp: 'site',
            rows: [['عدد الطبقات', nLift + ' طبقة'],
              ['سماكة الطبقة', (liftT * 1000).toFixed(0) + ' مم مدكوكة'],
              ['السماكة الكلية', (subT * 1000).toFixed(0) + ' مم'],
              ['لماذا طبقات', '**200 مم أقصى ما يخترقه الحادل**: الطبقة الأسمك '
                + 'يبقى أسفلها رخواً فيهبط الأساس عليه لاحقاً'],
              ['الحجم', subV.toFixed(1) + ' م³'],
              ['الدكّ', 'كل طبقة تُدكّ وحدها قبل فرش التي فوقها'],
              ['المرحلة', '٣ · السبيس']] });
      }
      // ٣) علامة الدكّ: شريط الحادل على سطح آخر طبقة
      const yTopS = fb - bl - .004;
      C.slab(gCmp, ar.w * .96, .012, ar.d * .96, ar.x, yTopS, ar.z,
        C.mat('soil', { rx: ar.w, clip: clip }),
        ai ? null : { title: 'الحدل (الدكّ)', kind: 'site', grp: 'site',
          rows: [['الأداة', 'حادل هزّاز — والدكّاكة اليدوية بالزوايا الضيّقة'],
            ['المرور', '4–6 مرّات لكل طبقة بتراكب 30% بين المسارات'],
            ['المطلوب', 'كثافة ≥ 95% من بروكتور المعدَّل (ASTM D1557)'],
            ['الفحص', 'مخروط الرمل أو النووي لكل طبقة قبل فرش التي فوقها'],
            ['لماذا يُعتمد', 'الهبوط الذي يظهر بعد سنة سببه طبقة لم تُدكّ، '
              + 'لا حسابٌ خاطئ'],
            ['المرحلة', '٤ · الحدل']] });
    });

    /* ٤) الضنبان (خرسانة النظافة) — يتبع شكل الأساس أيضاً */
    const gBl = coGrp('blind');
    const blAreas = (digKind === 'raft' && rf)
      ? [{ x: 0, z: 0, w: rf.Lx + .2, d: rf.Ly + .2 }]
      : digKind === 'piles'
        ? [{ x: 0, z: 0, w: L + .2, d: B + .2 }]
        : fts.map(f => ({ x: f.x, z: f.z, w: f.B + .2, d: f.B + .2 }));
    blAreas.forEach((ar, i) => C.slab(gBl, ar.w, bl, ar.d, ar.x, fb - bl / 2, ar.z,
      ...
      i ? null : { title: 'الضنبان — خرسانة النظافة', kind: 'site', grp: 'site',
        rows: [['السماكة', (bl * 1000).toFixed(0) + ' مم'],
          ...
          ['الشكل', digKind === 'raft' ? 'لوح واحد تحت الحصيرة كاملة'
            : digKind === 'piles' ? 'لوح واحد تحت الهامات' : 'رقعة تحت كل قاعدة'],
          ['المرحلة', '٥ · الضنبان']] }));
        ...
        C.formBox(gCf, cb2, ch2, hcol, X, z0 + hcol / 2, Z,
          { clip: clip, info: i ? null : { title: 'شدّة الأعمدة — طابق ' + f2,
            ...
            rows: [['المقطع', md.col.b + ' × ' + md.col.h + ' مم'],
              ...
              ['الجنائب', 'كل 55 سم رأسياً — وهي صفوف المرابط نفسها'],
              ['المرابط (العصافير)', 'كل ≤ 60 سم على الجنيب، وطرفاه مربوطان '
                + 'دائماً. تمنع **انتفاخ** القالب تحت الضغط الجانبي للخرسانة '
                + 'الطرية، والانتفاخ يخرج العمود منتفخ الوسط'],
              ['الجكّات', 'اثنان مائلان لكل وجه إلى ثلثَي الارتفاع بميل نحو 60° — '
                + 'وظيفتها **الشاقولية والثبات الجانبي** أثناء الصبّ والهزّ، '
                + 'لا مقاومة الضغط (تلك وظيفة المرابط)'],
              ['الفكّ', 'بعد 24–48 ساعة للأعمدة — وهي لا تحمل وزناً، فالفكّ '
                + 'مبكّر بخلاف شدّة السقف'],
              ['المرحلة', 'شدّة أعمدة الطابق ' + f2]] } });
  ...
  const ret = {
    ...
    /* يمرّ على كل مجسّمات التسليح — للفحص والتدقيق الآلي */
    eachRebar: fn => { buildRebar();
      [G.rebar, G.extra, G.chairs].forEach(g2 => g2.traverse(o => fn(o)));
      return true; },
    ...
    movieScenes: () => {
      ...
      /* ترتيب المراحل = **ترتيب التنفيذ بالموقع**:
         أرض ← تربة (نوعها) ← حفر ← سبيس طبقةً طبقة ← حدل ← ضنبان ←
         حديد وبسكويت ← شدّة ← صبّ ← جسور رابطة ← ردم ← الطوابق.
         وكان يقفز من الحفر إلى الضنبان، فيُصبّ الأساس على تربة لم تُعالَج. */
      const fKind = M.recommended || 'isolated';
      const fName = fKind === 'raft' ? 'حصيرة' : fKind === 'piles' ? 'ركائز'
        : 'أسس منفردة';
      const bed = ((M.soil && M.soil.beds) || []).filter(x => x.bearing)[0];
      const soilNm = (bed && bed.name) || (M.input && M.input.soil) || 'التربة الحاملة';
      const soilQa = (bed && bed.qa) || (M.input && M.input.qa) || null;
      add({ id: 'land', title: '١ · الأرض الطبيعية', sub: 'الموقع قبل أي عمل',
        add: ['ground'], cam: eye(T0, 1.15, .8, 22), look: T0, dur: 3.4, spin: .35 });
      add({ id: 'soilid', title: '٢ · التربة — على ماذا سنبني',
        sub: soilNm + (soilQa ? ' · تحمّل مسموح ' + Math.round(soilQa) + ' kPa' : '')
             + ' ← ' + fName,
        add: ['ground'], soil: true,
        cam: eye(T0, 1.05, 2.2, 18), look: T0, dur: 4.0, spin: .2 });
      add({ id: 'dig', title: '٣ · الحفر',
        sub: (fKind === 'raft' ? 'حفر عام تحت الحصيرة كاملة'
              : fKind === 'piles' ? 'حفر عام إلى منسوب بطن الهامات'
              : 'حفرة لكل قاعدة') + ' بعمق ' + digD.toFixed(2) + ' م',
        add: ['dig'], drop: ['ground'], soil: false,
        cam: eye(TF, (fKind === 'isolated' ? .85 : 1.35), .8, 46), look: TF,
        dur: 3.6, spin: .25 });
      /* الحفر العام للحصيرة والركائز يبلغ مسقط المبنى كلّه، فالكاميرا القريبة
         تقع **داخل** الردم فلا يُرى إلا نسيج الرمل. البُعد هنا يتّسع بحسب
         نوع الأساس لا بقيمة واحدة لكل الحالات. */
      const wide = (fKind === 'isolated') ? 1.0 : 1.5;
      add({ id: 'subbase', title: '٤ · فرش السبيس طبقةً طبقة',
        sub: 'طبقات ≤ 200 مم — لأن الحادل لا يخترق أكثر',
        add: ['subbase'], cam: eye(TF, .95 * wide, 1.5, 40), look: TF, dur: 3.6, spin: .2 });
      add({ id: 'compact', title: '٥ · الحدل',
        sub: 'كل طبقة تُدكّ وحدها حتى 95% بروكتور معدَّل قبل التي فوقها',
        add: ['compact'], cam: eye(TF, .90 * wide, 2.6, 36), look: TF, dur: 3.2 });
      add({ id: 'blind', title: '٦ · الضنبان — خرسانة النظافة',
        sub: 'سطح نظيف يُفرش عليه الحديد ويمنع امتصاص التربة لماء الخلطة',
        add: ['blind'], cam: eye(TF, .85 * wide, 1.9, 42), look: TF, dur: 3.4 });
      add({ id: 'reb_f', title: '٧ · حديد الأسس والبسكويت وأشاير الأعمدة',
        sub: 'الشبكة السفلى على بسكويت يحفظ الغطاء 75 مم، ثم أقفاص الانتظار',
        add: [], rebar: ['isolated', 'raft', 'piles'], extra: true,
        cam: eye(TF, .80 * wide, 2.4, 40), look: TF, dur: 4.0, spin: .3 });
      add({ id: 'form_f', title: '٨ · شدّة الأسس',
        sub: 'بليوود 25 مم بجنائب كل 55 سم · جكّات ومرابط (عصفور)',
        add: ['form_f'], rebar: ['isolated', 'raft', 'piles'],
        cam: eye(TF, .85 * wide, 0.5, 34), look: TF, dur: 3.2 });
      add({ id: 'conc_f', title: '٩ · صبّ ' + fName, sub: 'ثم تُفكّ الشدّة',
        add: ['conc_f'], rebar: false, drop: ['form_f'],
        cam: eye(TF, .95 * wide, .8, 42), look: TF, dur: 3.2, spin: .25 });
      add({ id: 'form_gb', title: '١٠ · شدّة الجسور الأرضية',
        sub: 'الجسور الرابطة تمنع حركة الأسس النسبية', add: ['form_gb'],
        cam: eye(P(0, fb + .5, 0), .85, 1.2, 34), look: P(0, fb + .5, 0), dur: 3.0 });
      add({ id: 'gbeam', title: '١١ · صبّ الجسور الأرضية', sub: 'وفكّ الشدّة',
        add: ['gbeam'], drop: ['form_gb'],
        cam: eye(P(0, fb + .5, 0), .95, 2.1, 40), look: P(0, fb + .5, 0), dur: 3.0 });
      add({ id: 'sand', title: '١٢ · الردم الرملي والدكّ',
    ...
    playScene: (s2, smooth) => {
      ...
      if (s2.site) {
        ...
        /* مشهد «التربة»: تُبنى الطبقات وتُظهر بأسمائها ومنسوب الطبقة الحاملة،
           فيرى المشاهد **على ماذا** سيُبنى قبل أن تُحفر الأرض. وبقية مشاهد
           الموقع تُطفئها لأن التربة تُحجب بالحفر. */
        if (s2.soil) { buildSoil(); G.soil.visible = true; }
        else if (G.soil) G.soil.visible = false;
      } else {
        siteMode(false);
